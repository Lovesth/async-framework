...
namespace async_framework::ranges
{
...
private:
    ...
    class iterator;
    static_assert(std::same_as<std::remove_cvref_t<value>, value> && 
    std::is_object_v<value>, "generator's value type must be a cv-unqualified object type");
    static_assert(std::is_reference_v<reference> || (std::is_object_v<reference> && std::same_as<std::remove_cv_t<reference>, reference> && std::copy_constructible<reference>), 
    "generator's second argument must be a reference typr or a cv-unqualified object type");
...
public:
    ...
    [[nodiscard]] std::default_sentinel end() const noexcept {return {};}
...
protected:
    ...
    template <typename R2, typename V2, typename Alloc2>
    struct NestedAwaiter {
        ...
        std::coroutine_handle<> await_suspend(std::coroutine_handle<Promise> handle) noexcept {
            auto target = std::coroutine_handle<gen_promise_base>::from_address(gen_.coro_.address());
            nested_.parent_ = std::coroutine_handle<gen_promise_base>::from_address(handle.address());
            auto& current = handle.promise();

            if(current.info_){
                nested_.root_ = current.info_->root;
            }else{
    ...
    friend class async_framework::coro::Generator;

    std::add_pointer_t<yielded> value_ = nullptr;
    std::coroutine_handle<gen_promise_base> top_ = std::coroutine_handle<gen_promise_base>::from_promise(*this);
    NestInfo* info_ = nullptr;
};
}// namespace detail

template<typename Ref, typename V, typename Allocator>
class Generator<Ref, V, Allocator>::promise_type : public detail::gen_promise_base<yielded>,
...
public:
	...
	auto final_suspend() noexcept {
		return typename Base::FinalAwaiter{};
	}

	std::suspend_always yield_value(yielded val) noexcept {
		this->value_ = addressof(val);
        return {};
	}

    auto yield_value(const std::remove_reference_t<yielded>& lval) requires
    std::is_rvalue_reference_v<yielded> && std::constructible_from<std::remove_cvref_t<yielded> && const std::remove_reference_t<yielded>& >
    {
        return typename Base::ElementAwaiter{lval};
    }

    template<typename R2, typename V2, typename Alloc2, typename Unused>
    ...
    auto yield_value(ranges::elements_of<R, Alloc> r) noexcept {
        ...
        ranges::internal::sentinel_t<R> s) -> Generator<yielded, ranges::internal::range_value_t<R>, Alloc> {
            for(; i!=s; ++i){
