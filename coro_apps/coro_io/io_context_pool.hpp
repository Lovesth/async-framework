...
namespace coro_io
{
    ...
    class ExecutorWrapper : public async_framework::Executor
    {
    private:
    ...
    public:
        ...
        auto get_asio_executor() const
        {
            return executor_;
        }

        operator ExecutorImpl()
        {
            return executor_;
        }
    ...
    inline async_framework::coro::Lazy<typename ExecutorImpl::executor_type>
    get_current_executor()
    {
        auto executor = co_await async_framework::CurrentExecutor{};
    ...
    public:
            ...
            : next_io_context_(0), cpu_affinity_(cpu_affinity)
        {
            ...
            for (size_t i = 0; i < pool_size; ++i)
            {
                ...
                auto executor = std::make_unique<coro_io::ExecutorWrapper<>>(io_context->get_executor());
        ...
        {
            auto i = next_io_context_.fetch_add(1, std::memory_order_relaxed);
        ...
        static size_t get_total_thread_num()
        {
    ...
    private:
        ...
        // const and inline static can be initialized in class
    ...
    public:
        multithread_context_pool(size_t thd_num = std::thread::hardware_concurrency())
            : work_(std::make_unique<asio::io_context::work>(ioc_)), executor_(ioc_.get_executor()),
              thd_num_(thd_num) {}
        ~multithread_context_pool() {}
    ...
    {
        static auto g_io_context_pool_ = std::make_shared<T>(pool_size);
        ...
        return *g_io_context_pool_;
    }

    template <typename T = io_context_pool>
    inline std::shared_ptr<T> create_io_context_pool(unsigned pool_size = std::thread::hardware_concurrency())
    {
    ...
    template <typename T = io_context_pool>
    inline T &g_block_io_context_pool(unsigned pool_size = std::thread::hardware_concurrency())
    {
        static auto g_io_context_pool_ = std::make_shared<T>(pool_size);
        [[maybe_unused]] static bool run_helper = [](auto pool)
        {
    ...
    template <typename T = io_context_pool>
    inline auto get_global_block_executor(unsigned pool_size = std::thread::hardware_concurrency())
    {
