...
class rate_limiter {
 public:
  async_framework::coro::Lazy<std::chrono::milliseconds> acquire(int permits) {
  ...
  async_framework::coro::Lazy<void> set_rate(double permitsPerSecond) {
    auto scope = co_await this->lock_.coScopedLock();
 ...
 private:
  ...
  async_framework::coro::SpinLock lock_;
};
...
class smooth_bursty_rate_limiter : public abstract_smooth_rate_limiter {
 public:
  smooth_bursty_rate_limiter(double permits_per_second) {
    ...
    async_framework::coro::syncAwait(set_rate(permits_per_second));
  }
